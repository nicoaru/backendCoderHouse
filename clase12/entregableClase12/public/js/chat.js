...
const handleSendMessage = (e) => {
    if(email) {
        ...
        const msg = {mensaje, time, author: email}
...
const addMessage = (msgObject, container) => {
    ...
    const msgHtml = `<span style="color: green"><i>${msgObject.mensaje}</i></span>`
    const timeHtml = `<span style="color: brown">${msgObject.time}</span>`
    container.innerHTML+= `<div id="msg${msgObject.id}" class="bg-light border d-flex justify-content-between p-3"> <div>${authorHtml} ${msgHtml}</div> <div>${timeHtml}</div></div>`
